    ...
    @foreach($allReports as $stampCode => $reports)
        ...
        @foreach($reports as $index => $report)
            ...
            <table class="border-none" style="margin: 20px 0px 0px 0px">
                ...
                <tr>
                    <th colspan="3" class="left-align">เป็นจำนวนตราประทับ</th>
                    <th colspan="1">จำนวน</th>
                    <th colspan="1">{{ ($total['totalQuota'] < $total['stampHours']) ? number_format($total['calculateHoursSpentExceeded'], 0) : '-' }}</th>
                    <th colspan="1">ดวง</th>
                    <th colspan="1"></th>
                    <th colspan="1"></th>
                </tr>
            ...
            <table class="border-none" style="margin: 20px 0px 0px 0px">
                <tr>
                    <th colspan="3" class="left-align">ท่านได้ใช้ชั่วโมงในตราประทับทั้งสิ้น</th>
                    <th colspan="1">จำนวน</th>
                    <th colspan="1">{{ $total['stampRemainingHours'] > 0 ? number_format($total['stampRemainingHours'], 0) : '0' }}</th>
                    <th colspan="1">ชั่วโมง</th>
                    <th colspan="1">0</th>
                    <th colspan="1">นาที</th>
                </tr>
                ...
                <tr>
                    <th colspan="3" class="left-align">คิดเป็นเงิน ยอดคงเหลือที่ต้องชำระ</th>
                    <th colspan="1">จำนวน</th>
                    <th colspan="1">{{ $total['stampRemainingHours'] > 0 ? number_format($total['stampRemainingHours'] * $total['stampCharge'], 0) : '0' }}</th>
                    <th colspan="1">บาท</th>
                </tr>
            </table>
    ...
    <table border="1" style="font-size: 14px !important;" class="no-border-outside">
        ...
        <tbody>
            ...
            <tr>
                ...
                <th class="bordered" colspan="1">{{ $total['totalRemainingHours'] }}:{{ $total['totalRemainingMinutes'] }}:{{ $total['totalRemainingSeconds'] }}</th>
    ...
    <div class="container">
        ...
        <table class="border-none" style="margin: 30px 0px 0px 0px">
            ...
            <tr>
                ...
                <td colspan="1" class="left-align">{{ number_format($total['stampHours'], 0) }} - {{ number_format($total['totalRemainingHours'], 0) }}</td>
            ...
            @if($total['totalQuota'] > 0)
                ...
                <tr>
                    <td colspan="2" class="left-align"></td>
                    <td colspan="1" class="left-align">{{ number_format($total['hoursSpentExceeded'], 0) }} / {{ number_format($total['stampCondition'], 0) }}</td>
                    <td colspan="4" class="left-align">ผลที่ได้นำมาหาร 2 ชั่วโมง (ตราประทับ 1 ดวงจอดรถได้ 2 ชั่วโมง)</td>
                </tr>
                <tr>
                    <td colspan="2" class="left-align"></td>
                    <td colspan="1" class="left-align">{{ number_format($total['calculateHoursSpentExceeded'], 0) }} x {{ number_format($total['stampCharge'], 0) }} บาท</td>
                    <td colspan="4" class="left-align">จำนวนชั่วโมงจอดเกิน x จำนวนเงินค่าใช้ตราประทับที่บริษัทกำหนด</td>
                </tr>
                <tr>
                    <td colspan="2" class="left-align"></td>
                    <td colspan="1" class="left-align">{{ number_format($total['totalParkingFee'], 0) }} บาท</td>
                    <td colspan="4" class="left-align">จำนวนเงินต้องชำระ ไม่รวม vat {{ number_format($total['vatPercent'], 0) }}%</td>
                </tr>
            @else
                <tr>
                    <td colspan="2" class="left-align"></td>
                    <td colspan="1" class="left-align">{{ number_format($total['stampRemainingHours'], 0) }} x {{ number_format($total['stampCharge'], 0) }} บาท</td>
                    <td colspan="4" class="left-align">จำนวนชั่วโมงจอดเกิน x จำนวนเงินค่าใช้ตราประทับที่บริษัทกำหนด</td>
                </tr>
                <tr>
                    <td colspan="2" class="left-align"></td>
                    <td colspan="1" class="left-align">{{ number_format($total['stampRemainingHours'] * $total['stampCharge'], 0) }} บาท</td>
                    <td colspan="4" class="left-align">จำนวนเงินต้องชำระ ไม่รวม vat {{ number_format($total['vatPercent'], 0) }}%</td>
                </tr>
            @endif
