			...
			<form id="addPettyCashForm" method="POST">
				<div class="row py-4 px-1" style="background-color: #EDEDEDFF">
					...
					<div class="col-md-4 pt-3">
						<label for="pcash_date" class="mb-1">วันที่ออกใบทดลองจ่าย</label>
						<div class="input-group">
							<div class="input-group-prepend">
								<i class="input-group-text">
					...
					<div class="col-md-3 pt-3">
						<label for="pettyCashList" class="mb-1 d-flex align-items-center">
							...
							<input type="hidden" name="pcash_type" id="pcash_type" value="2"> 
		...
		$(document).ready(function() {
			$('#previewPettyCash').click(function(e) {
				...
				if(totalTaxList > 0) {
					...
					taxNos.forEach((taxNo, index) => {
						if (taxNo) {
							const taxListItem = `<li style="font-size: 12px;">เลขที่ใบกำกับภาษี <span><b>${taxNo}</b></span> (<span>${taxDates[index]!= '' ? formatDateTH(taxDates[index]) : 'ไม่ได้เลือกวันที่'}</span>)</li>`;
			...
			$('#savePettyCashForm').click(function(e) {
				...
				if (!pettyCashDate) {
					errorMessages.push("กรุณาเลือกวันที่ออกใบทดลองจ่าย");
				}
				if (!payableId) {
						...
						success: function(response) {
							if(response.status == 'success') {
								swal({
									...
									text: "เลขที่ใบทดลองจ่าย : " + response.pCashNo,
									type: "success",
									closeOnClickOutside: false
								},function() {
									window.location.href = "advance.php?cid=<?=$cid?>&dep=<?=$dep?>";
								});
							}
							else {
								swal({
