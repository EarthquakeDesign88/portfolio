		...
		<div class="container">
			...
			<form method="POST" name="frmOutputTax" id="frmOutputTax" action="">
				...
				<div class="row py-4 px-1" style="background-color: #E9ECEF">
					<div class="col-md-12">
						<h3 class="mb-0">
							<i class="icofont-prescription"></i>&nbsp;&nbsp;ออกรายงานภาษีซื้อ
						</h3>
					</div>
							...
							<div class="card-body">
								...
								<!-- <?php if($cid === "C001") { ?>
									<div class="text-center d-flex justify-content-between align-items-center">
										<div>
											<input type="checkbox" id="AD" name="AD" value="AD"> AD
										</div>
										<div>
											<input type="checkbox" id="AR" name="AR" value="AR"> AR
										</div>
										<div>
											<input type="checkbox" id="CI" name="CI" value="CI"> CI
										</div>
										<div>
											<input type="checkbox" id="CM" name="CM" value="CM"> CM
										</div>
									</div>
								<?php } ?> -->
		...
		$(document).ready(function () {
			...
			$("#btnSearch").click(function() {
				if($('#datefrom').val() == '') {
					...
					$.ajax({
						type: "POST",
						url: "r_report_tax.php",
						data: $("#frmOutputTax").serialize(),
						success: function(result) {
							if(result.status == 1) {
								window.location = "tax_report_output.php?cid=" + result.compid + "&dep=" + result.depid + "&df=" + result.dateFrom + "&dt="+ result.dateTo;
							} else {
