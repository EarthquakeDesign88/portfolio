...
// $str_sql = "SELECT * FROM project_tb AS p
// 			WHERE proj_compid = '". $_POST['queryComp'] ."' AND proj_depid = '". $_POST['queryDep'] ."'";
...
LEFT JOIN 
    (SELECT 
        invrcptD_projid,
        CASE 
            WHEN MIN(invrcptD_status) = '0' THEN '0'
            ELSE '1'
        END AS invrcptD_status
    FROM 
        invoice_rcpt_desc_tb
    GROUP BY 
        invrcptD_projid) AS i 
ON 
    p.proj_id = i.invrcptD_projid
WHERE 
    p.proj_compid = '" . $_POST['queryComp'] . "' 
    AND p.proj_depid = '" . $_POST['queryDep'] . "'";

if ($_POST['query'] != '') {
	$str_sql .= ' AND proj_name LIKE "%' . str_replace(' ', '%', $_POST['query']) . '%" ';
}

$str_sql .= "GROUP BY proj_id";
...
if ($total_data > 0) {
	...
	foreach ($obj_query as $obj_row) {
		$sum = $obj_row["invrcptD_status"] == '0' ? 'แจ้งหนี้ไม่ครบ' : 'แจ้งหนี้ครบแล้ว';
		$color = $obj_row["invrcptD_status"] == '0' ? 'danger' : 'success';
		$output .= '<tr>
							<td>
								' . $obj_row["proj_id"] . '
							</td>
							<td>
								' . $obj_row["proj_name"] . '<br>

							</td>
							<td>
								<div class="text-light bg-' . $color . ' text-center p-1">' . $sum . '</div><br>
