				...
				<div class="col-md-12 mr-auto mb-5">
					<article>
						<p class="subtitle fancy">
							<span>ใบแจ้งหนี้ ( เกินกำหนดชำระ )</span>
						</p>
					</article>
					...
					<div class="row">
						...
						<?php
							...
							while ($obj_row = mysqli_fetch_array($obj_rs)) {
								...
								if($obj_row_user["user_levid"] == '2') {
									$str_sql_iv = "SELECT DISTINCT * FROM invoice_tb AS i WHERE inv_rev = (SELECT MAX(inv_rev) AS inv_rev FROM invoice_tb WHERE inv_runnumber = i.inv_runnumber) AND inv_statusMgr = 1 AND inv_apprMgrno <> '' AND inv_depid = '". $obj_row["dep_id"] ."' AND Now() > inv_duedate GROUP BY inv_runnumber";
								} else {
									$str_sql_iv = "SELECT DISTINCT * FROM invoice_tb AS i WHERE inv_rev = (SELECT MAX(inv_rev) AS inv_rev FROM invoice_tb WHERE inv_runnumber = i.inv_runnumber) AND inv_statusMgr = 0 AND inv_apprMgrno = '' AND inv_depid = '". $obj_row["dep_id"] ."' AND Now() > inv_duedate GROUP BY inv_runnumber";
								}
						...
						<div class="col-md-3 my-3">
							<a href="duedate.php?cid=<?=$cid;?>&dep=<?= $obj_row["dep_id"]; ?>">
