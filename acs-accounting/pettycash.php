...
$html_dep_box = __html_dep_box($html_title,'pettycash.php',$icon,"AND pcash.pcash_type = 1",$arrDepAll,$sql,"pcash.pcash_dep_id");
...
<body>
		...
		<div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
			<div class="modal-content">
				<div class="modal-header">
					<h3 class="modal-title py-2">รายละเอียดใบจ่ายเงินสดย่อย <span id="pcashHeader"></span></h3>
	...
	<script type="text/javascript">
						...
						<tr>
							...
							<td class="text-center">
								<div class="btn-group btn-group-toggle">
									<a href="pettycash_edit.php?cid=C001&dep=D003&pCashId=${item.pCashId}" class="btn btn-warning edit_data" title="แก้ไข / Edit">
		...
		function printPettyCash(pCashId, pCashNo) {
			$.ajax({
				...
				success: function(response) {
					...
					a.href = blobUrl;
					a.download = 'ใบจ่ายเงินสดย่อย ' + pCashNo + '.pdf';
		...
		function softDelete(pCashId, pCashNo) {
			swal({
				title: "ยืนยันการลบ?",
				text: "คุณต้องการลบใบจ่ายเงินสดย่อย เลขที่: " + pCashNo + " หรือไม่?",
				type: "warning",
						...
						success: function(response) {
							if (response.status === "success") {
								swal({
									...
									text: "เลขที่ใบจ่ายเงินสดย่อย : " + pCashNo,
									type: "success",
									closeOnClickOutside: false
								}, function() {
