<?php
	...
	$str_sql = "SELECT * FROM invoice_tb AS i INNER JOIN payable_tb AS p ON i.inv_payaid = p.paya_id INNER JOIN company_tb AS c ON i.inv_compid = c.comp_id INNER JOIN department_tb AS d ON i.inv_depid = d.dep_id INNER JOIN payment_tb AS paym ON i.inv_paymid = paym.paym_id WHERE paym_stsid = 'STS002' AND inv_compid = '". $_POST['queryComp'] ."' AND paym_month = '". $_POST['queryMonth'] ."' ";

	$str_sql .= " ORDER BY paym_paydate ASC ";
	...
	if($total_data > 0) {
		...
		foreach($obj_query as $obj_row) {
			...
			$output .= '<tr>
							<td class="text-center">
								'. DateShortThai($obj_row['paym_paydate']) .'
							</td>
							<td>
								<table width="100%">
									<tr>
										...
										<td width="20%" style="border: none; padding: 0px; text-align: right;">
											<b>'. number_format($obj_row["inv_grandtotal"],2) .'</b>
										</td>
									</tr>
								</table>
							</td>
		...
		$output .= '</tbody>
					<tbody>
						<tr width="100%">
							<td colspan="2" align="center">ไม่มีรายการรายจ่ายเดือนนี้</td>
						</tr>
					</tbody>';

	}

		$str_sql_sum = "SELECT * FROM invoice_tb AS i INNER JOIN payable_tb AS p ON i.inv_payaid = p.paya_id INNER JOIN company_tb AS c ON i.inv_compid = c.comp_id INNER JOIN department_tb AS d ON i.inv_depid = d.dep_id INNER JOIN payment_tb AS paym ON i.inv_paymid = paym.paym_id WHERE paym_stsid = 'STS002' AND inv_compid = '". $_POST['queryComp'] ."' AND paym_month = '". $_POST['queryMonth'] ."'";
		$obj_rs_sum = mysqli_query($obj_con, $str_sql_sum);
		$Sumsub = 0;
		$Sumvat = 0;
		$Sumgrand = 0;
		while($obj_row_sum = mysqli_fetch_array($obj_rs_sum)) {
			$Sumsub = $obj_row_sum["inv_subtotal"] + $Sumsub;
			$Sumvat = $obj_row_sum["inv_vat"] + $Sumvat;
			$Sumgrand = $obj_row_sum["inv_grandtotal"] + $Sumgrand;
		}
