<?php
	...
	$comp =  (isset($_POST['queryComp'])) ? $_POST['queryComp'] : "";
	$dep_id = (isset($_POST['queryDep'])) ? $_POST['queryDep'] : "";
	$df =  (isset($_POST['df'])) ? $_POST['df'] : "";
	$dt =  (isset($_POST['df'])) ? $_POST['dt'] : "";
	$title =  (isset($_POST['title'])) ? base64_decode($_POST['title']) : "";

	$str_sql = "SELECT * FROM invoice_tb AS i INNER JOIN payment_tb AS paym ON i.inv_paymid = paym.paym_id INNER JOIN payable_tb AS p ON i.inv_payaid = p.paya_id WHERE inv_compid = '". $comp ."' "; 
	$str_sql .= ($dep_id!="") ? " AND paym_depid = '". $dep_id ."'" : "";
	
	if($df!="" && $dt!=""){
		$str_sql .= " AND paym_date BETWEEN '". $df ."' AND '". $dt ."' ";
	}else{
		if($df!=""){
			$str_sql .= " AND DATE(paym_date) >= '". $df ."'";
		}
	...
	$_SESSION["session_report_payment_print"] = array(
		"result" =>  $db->query($str_sql)->result(),
		"title" => $title,
	);
	...
	$output = '<table class="table table-bordered mb-0">
					<thead class="thead-light">
						<tr>
							<th width="12%">เลขที่ใบสำคัญจ่าย</th>
