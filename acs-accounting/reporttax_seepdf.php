...
<?php
	...
	if (!$_SESSION["user_name"]){  //check session
		...
		$str_sql = "SELECT * FROM taxpurchase_tb WHERE tax_comp_id = '$cid' AND tax_dep_id = '$dep'";
				...
				<div class="row py-4 px-1" style="background-color: #e9ecef">
					...
					<div class="col-md-4">
						<div class="row text-right">
							<div class="col-md-12">
								<a href="reporttax_seepdf.php?cid=<?=$cid;?>" type="button" class="btn btn-warning" name="btnBack" id="btnBack">
				...
				<div class="row py-4 px-1" style="background-color: #FFFFFF">
					<div class="col-md-2">
						...
						<div class="nav flex-column nav-pills nav-pills-custom" id="v-pills-tab" role="tablist" aria-orientation="vertical">
							...
							<?php
								$str_sql_y = "SELECT YEAR(tax_created_at) + 543 as tax_year FROM taxpurchase_tb GROUP BY tax_year";
								$obj_rs_y = mysqli_query($obj_con, $str_sql_y);
								$i = 1;
                                $count = mysqli_num_rows($obj_rs_y);
								if($count > 0) {
                                    while ($obj_row_y = mysqli_fetch_array($obj_rs_y)) {
                                        $yearTH = $obj_row_y["tax_year"];
							?>

							<a class="nav-link mb-3 p-3 shadow <?php if ($yearTH == $y) echo "active"; ?>" id="y<?=$obj_row_y["tax_year"]?>-tab" href="reporttax_seepdf.php?cid=<?=$cid?>&dep=<?=$dep?>&y=<?=$yearTH?>&m=<?=$m?>" role="tab" aria-controls="v-pills-home" aria-selected="true">
								<i class="fa fa-user-circle-o mr-2"></i>
								<span class="font-weight-bold  text-uppercase">
									พ.ศ. <?=$obj_row_y["tax_year"]?>
								</span>
							</a>
					...
					<div class="col-md-10">
						<div class="col-md-12 d-none">
							...
							<input type="text" class="form-control" name="mt" id="mt" value="<?=$m_para;?>">
						...
						<div class="tab-content" id="v-pills-tabContent">
							<div class="fade shadow rounded bg-white show p-4">
								...
								<ul class="nav nav-tabs nav-fill" role="tablist">
									...
									?>
									<li class="nav-item">
										<a class="nav-link <?php if ($monthNo == $m_para) echo "active"; ?>" href="reporttax_seepdf.php?cid=<?=$cid;?>&dep=<?=$dep;?>&y=<?=$yearTH;?>&m=<?=$m;?>" style="width: 100%" title="<?=$monthNo;?>">
											<?php 
												switch ($month) {
													case 'Jan':
														$monthTH = 'ม.ค.';
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			function load_data(page, query = '', dep = '', y = '', m = '', comp = '') {
				...
				var comp = $('#compid').val();
				$.ajax({
					url:"fetch_reporttax_seepdf.php",
					method:"POST",
