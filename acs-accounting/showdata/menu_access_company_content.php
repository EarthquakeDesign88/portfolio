...
$sql2 = "SELECT *  FROM department_tb WHERE dep_compid='".$comp_id."'";
...
$html_dep .= "<tr>";
	$html_dep .= "<th>ID</th>";
	$html_dep .= "<th>ชื่อ</th>";
	$html_dep .= "<th>ประเภท</th>";
	$html_dep .= "<th>สถานะ</th>";
...
if(count($result2)>=1){
	foreach ($result2 as $row2) {
		...
		$dep_compid = $row2["dep_compid"];
		$dep_code = $row2["dep_code"];
		...
		$html_dep .= "<tr>";
			$html_dep .= "<td>".$dep_id."</td>";
			$html_dep .= "<td>".$dep_name."</td>";
			$html_dep .= "<td>".$dep_type."</td>";
			
		if($dep_status){
			$html_dep .= "<td>ใช้งาน</td>";
		}else{
			$html_dep .= "<td>ปิดใช้งาน</td>";
		}
	}
}
...
$arrAccess = __menu_list_access_company($comp_id);
...
<table>
	...
	<tr>
		 <td>
		 	<b>Name:</b> <?=$row["comp_name"];?>
		 </td>
	</tr>
	<tr>
		 <td>
		 	<b>Name Short:</b> <?=$row["comp_name_short"];?>
		 </td>
	</tr>
	<tr>
		 <td>
		 	<b>แผนกภายในบริษัท:</b> <br>
		 	<?=$html_dep;?>
		 </td>
	</tr>
  <tr>
...
<script>
	...
	function onSave(){
		...
		if(confirm("คุณต้องการบันทึกข้อมูล ใช่หรือไม่?")){
			...
			formData.append('action', "save_menu_access_company");
