<?php
	...
	$str_sql = "SELECT * FROM purchasereq_tb AS pr INNER JOIN purchasereq_list_tb AS prl ON pr.purc_no = prl.purclist_purcno INNER JOIN payable_tb AS p ON pr.purc_payaid = p.paya_id INNER JOIN department_tb AS d ON pr.purc_depid = d.dep_id WHERE purc_apprceono = '' AND purc_compid = '". $_POST['queryComp'] ."' AND purc_depid = '". $_POST['queryDep'] ."' ";
	...
	if($total_data > 0) {
		...
		foreach($obj_query as $obj_row) {
			...
			$output .= '<tr id="tr'. $pageno . $i .'" style="'.$chkbg.'">
							...
							<td class="d-none">
								<input type="text" name="purcstatusceo" id="purcstatusceo'. $obj_row["purc_id"] .'" value="'. $obj_row["purc_statusceo"] .'">
								<input type="text" name="purcapprceono" id="purcapprceono" value="'. $obj_row["purc_apprceono"] .'">
								<input type="text" name="purcid" id="purcid" value="'. $obj_row["purc_id"] .'">
							</td>
						</tr>';
		...
		$output .= '</tbody>
					...
					<tbody>
						<tr>
							<td colspan="5" class="text-center">ไม่มีข้อมูลขอซื้อขอจ้าง</td>
						</tr>
					</tbody>';
	...
	$output .= '<script type="text/javascript">
					...
					function ClickValue(frmApprovePR, chk, index, purcid, sts) {
						...
						var check = document.getElementById("purcstatusceo"+purcid).value;
						...
						if(check == 0) {
							...
							document.getElementById("purcstatusceo"+purcid).value = 1;
							...
							$.ajax({
								...
								type: "post",
								url: "r_fetch_purchaseReq_ceo_approve.php",
							...
							document.getElementById("purcstatusceo"+purcid).value = 0;
	...
	$output .= $previous_link . $page_link . $next_link;
			$output .= '</ul>
					...
					$str_sql_sum = "SELECT * FROM purchasereq_tb  WHERE purc_statusceo = 1 AND purc_apprceono = '' AND purc_compid = '". $_POST['queryComp'] ."' AND purc_depid = '". $_POST['queryDep'] ."'";
		...
		$output .= '<div class="col-md-6 px-0 py-4 text-right">
						
						<div class="row mx-0">
							<div class="col-md-7 px-0 pr-2 my-auto">
								<h4 class="mb-0 text-right"><b>ยอดรวมอนุมัติ</b></h4>
							</div>
							<div class="col-md-5 px-0">
								...
								<input type="text" class="form-control text-right d-none" id="NetamountInvSelNum" value="'. $sum .'" readonly>
