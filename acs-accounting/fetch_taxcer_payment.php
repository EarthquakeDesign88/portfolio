<?php
	...
	// } else {
		$str_sql = "SELECT DISTINCT * FROM invoice_tb AS i 
					...
					INNER JOIN department_tb AS d ON i.inv_depid = d.dep_id 
					INNER JOIN payment_tb AS paym ON i.inv_paymid = paym.paym_id  
					WHERE inv_rev = (SELECT MAX(inv_rev) AS inv_rev FROM invoice_tb WHERE inv_runnumber = i.inv_runnumber) AND inv_statusMgr = 1 AND paym_NostatusTaxcer = '' AND inv_depid = '". $_POST["queryDep"] ."' ";
	// }
	...
	$str_sql .= ' GROUP BY paym_no ORDER BY paym_no DESC ';
			...
			$output .= '<tr id="tr'. $i .'">
							...
							<td class="text-center">
								<div class="btn-group btn-group-toggle"  style="width: 100%">
									<a href="taxcer_payment_add.php?cid='. $obj_row['comp_id'] .'&dep='. $obj_row["dep_id"] .'&paymid='. $obj_row["paym_id"] .'&taxT=1" class="btn btn-primary" type="button" name="add" title="เพิ่ม / Add">
										<i class="icofont-plus-circle"></i>
									</a>
									
								</div>
								<a href="taxcer_payment_edit.php?cid='. $obj_row['comp_id'] .'&dep='. $obj_row["dep_id"] .'&paymid='. $obj_row["paym_id"] .'&taxT=1" class="btn btn-warning d-none" type="button" name="edit" title="แก้ไข / Edit">
